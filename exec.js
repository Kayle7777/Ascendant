...
class GetPost {
    localFiles; // [ { title: string, post: string } ]
    scrapedPost; // type
                 // title
                 // time
                 // fileName
                 // post
    isDuplicate; // Boolean; true if post is present on disk already
    updateLocalFiles() {
    ...
    init() {
        ...
        let toWrite = format.buildHtmlPipe(this.scrapedPost.post, this.scrapedPost.fileName);
